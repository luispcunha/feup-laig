...
class MySceneGraph {
    ...
    parseXMLFile(rootElement) {
        const nodeProperties = {
            scene: { index: SCENE_INDEX, parse: this.parseScene },
            views: { index: VIEWS_INDEX, parse: this.parseView },
            globals: { index: GLOBALS_INDEX, parse: this.parseGlobals },
            lights: { index: LIGHTS_INDEX, parse: this.parseLights },
            textures: { index: TEXTURES_INDEX, parse: this.parseTextures },
            materials: { index: MATERIALS_INDEX, parse: this.parseMaterials },
            transformations: { index: TRANSFORMATIONS_INDEX, parse: this.parseTransformations },
            primitives: { index: PRIMITIVES_INDEX, parse: this.parsePrimitives },
            components: { index: COMPONENTS_INDEX, parse: this.parseComponents }
        };
        ...
        for (let name of names) {
            ...
            if (index == -1)
                ...
                const info = nodeProperties[name];
                if (index != info.index)
                    this.onXMLMinorError("tag <" + name + "> out of order " + info.index);
                const error = info.parse(nodes[index]);
                if (error != null)
                    return error;
            }
        }
